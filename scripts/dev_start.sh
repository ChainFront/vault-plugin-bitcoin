...
echo "--> Building"
go build -o "$SCRATCH/plugins/vault-plugin-bitcoin"

echo "    Registering plugin"
SHASUM=$(shasum -a 256 "$SCRATCH/plugins/vault-plugin-bitcoin" | cut -d " " -f1)
vault write sys/plugins/catalog/bitcoin-plugin \
  sha_256="$SHASUM" \
  command="vault-plugin-bitcoin"

echo "    Mounting plugin"
vault secrets enable -path=bitcoin -plugin-name=bitcoin-plugin plugin
